...
filename_pre = "powerSpec/powerSpec_up_C_0.000000_Hann_K_"
specData = np.empty(0)
files = np.geomspace(fMin,fMax,numberOfFiles)
...
for it in range(0,numberOfFiles):
  specData[it] = gaussian_filter(specData[it],sigma=10.0)
...
plt.imshow(np.flipud(specData.T), aspect="auto", cmap=MyCmap, interpolation="none", extent=extentarray, norm=mpl.colors.LogNorm(), vmin = 1.0E-10, vmax=1E-1)
